...
class OfflineControllerWithSmallRotation(BaseController):
    ...
    def __init__(
            ...
            grid_size=0.25,
            fov=100,
            offline_data_dir='/home/chenhaobo996/hxb/Data/AI2Thor_offline_data_2.0.2/',
    ...
    def get_depth_value_add_detection(self, detection=None):
        from utils.data_utils import sarpn_depth_h5
        import torch

        depth = sarpn_depth_h5(str(self.state),str(self.scene_name)).squeeze(dim=0).squeeze(dim=0)
        for i in range(len(detection['indicator'])):
            if detection['indicator'][i] == 1:
            # if detection['labels'][i] != 0 and detection['labels'][i] != 23:                  # 尝试所有label的点都添加深度信息
                target_bbox = detection['bboxes'][i]    
                detection["x_dim"][i] = torch.tensor([target_bbox[1],target_bbox[3]])
                detection["y_dim"][i] = torch.tensor([target_bbox[0],target_bbox[2]]) 
                ...
                detection["depth"][i] = depth[int_target1:int_target3,int_target0:int_target2].mean()

                if detection["depth"][i] != detection["depth"][i]:
                    print("int_target0, int_target1, int_target2, int_target3 is {},{},{},{}".format(int_target0, int_target1, int_target2, int_target3))
                    print("detection_inputs['depth'][i] is {}".format(detection["depth"][i]))
                break
        return detection
